...
def main(input_path: str, output_path: str, mode: str, pages: str) -> None:
    ...
    if not slides_to_process_objects:
        if pages:  # User specified a range, but it resulted in no slides
            ...
            # No pages specified, but presentation was empty (already handled) or became
            # empty (should not happen here)
            click.echo(
        ...
        # This is important if only a copy was intended or if the range was valid but
        # empty.
        prs.save(output_path)
        click.echo(f"Presentation saved (no text modifications) to: {output_path}")
        # Save cache even if no text elements were processed, in case it was loaded and
    ...
    # The original logic for 'slides_for_text_extraction' initialization is now replaced
    # by the above.
    # The following 'if mode == "translate" or mode == "reverse-words":' block's content
    # related to populating slides_for_text_extraction is no longer needed here as
    # slides_to_process_objects is already set.
    ...
    if mode in {"translate", "reverse-words"} and slides_to_process_objects:
        click.echo(
            ...
            "slides (out of {num_original_slides} total) in the copied presentation..."
        )
    # 'duplicate-only' mode and slide duplication logic removed.
    # Text processing will now occur directly on the slides in
    ...
    if mode == "translate":
        ...
        # Stores items for LLM:
        # {id, original_text_for_cache, text_to_send, run_object, page_hash}
        global_texts_for_llm_prompt = []

        # Stores details for all runs:
        # {run_object, final_translation, from_cache, original_text,
        #  llm_id (if applicable)}
        all_processed_run_details = []

        # {page_hash: [{"original_text": ..., "translation": ...}]}
        ...
        if slides_to_process_objects:
            ...
            for slide_idx, slide_to_extract in enumerate(slides_to_process_objects):
                ...
                # List of {"original_text": ..., "run_object": ...}
                current_page_run_info = []

                # First pass: extract all texts from the current slide for hashing and
                # run info
                ...
                click.echo(
                    f"  Slide {slide_idx + 1}: Hash '{page_hash[:8]}...', "
                    f"{len(current_page_run_info)} text runs."
                )
                ...
                if page_hash in translation_cache:
                    ...
                    for run_detail in current_page_run_info:
                        ...
                        if not found_in_page_cache:
                            # This case implies the page hash matched, but an individual
                            # text on that page wasn't in the cached list for that page.
                            # This might happen if the page structure is identical but
                            # some minor text was edited, then re-edited back to make
                            # the hash match, but the cache entry for that page is from
                            # a state where that specific text was different. For
                            # simplicity, we'll treat this as needing LLM translation
                            # for this specific run.
                            click.echo(
                                f"    Partial page cache hit for {page_hash[:8]}. "
                                f"Text '{run_detail['original_text'][:30]}...' "
                            ...
                            global_texts_for_llm_prompt.append(
                                {
                                    ...
                                    + EOL_MARKER,
                                    "run_object": run_detail["run_object"],
                                    "page_hash": page_hash,  # Associate w/ current page
                                }
                            )
                            all_processed_run_details.append(
                                {
                    ...
                    click.echo(
                        ...
                        f"Will send {len(current_page_run_info)} runs to LLM."
                    )
                    pending_page_cache_updates[
                        page_hash
        ...
        if (
            ...
            # Save cache even if empty/unchanged, as it might have been loaded and
            # format changed
            click.echo(
                f"Saving cache (potentially empty or format updated) to: "
                f"{cache_file_path}"
            )
            save_cache(translation_cache, cache_file_path)
            return
        ...
        if not global_texts_for_llm_prompt:
            ...
            # Sort by ID to ensure consistent order for LLM prompt, if IDs are not
            # strictly sequential
            ...
            for orig_line in translated_text_response.splitlines():
                ...
                try:
                    ...
                    if len(parts) == 2:  # noqa: PLR2004
                        ...
                        if prompt_item_data:
                            ...
                            # Avoid duplicate entries if a text appears multiple times
                            # on a page and was sent to LLM (though current logic sends
                            # each run instance, so original_text might not be unique in
                            # the list for a page)
                            # For now, we assume each original_text within a page that
                            # went to LLM is distinct enough or handled by run_object
                            # uniqueness.
                            # The cache structure is a list of
                            # {"original_text": ..., "translation": ...} for the page.
                            ...
                            for detail_item in all_processed_run_details:
                                if detail_item.get("llm_id") == parsed_text_id:
                                    ...
                                    # We also need to prepare for saving this to the
                                    # page's cache entry
                                    # Check if this original_text is already slated for
                                    # this page_hash update
                                    ...
                                    for pending_item in pending_page_cache_updates[
                                        ...
                                        if (
                                            ...
                                            pending_item["translation"] = (
                                                # Update if somehow already there
                                                final_llm_translation
                                            )
                            ...
                            click.echo(
                                "Warning: Could not find original data "
                                f"for ID {parsed_text_id} from LLM response.",
                                err=True,
                            )
                    else:
                        click.echo(
        ...
        for page_hash, translations_list in pending_page_cache_updates.items():
            ...
            # The current logic for `pending_page_cache_updates[page_hash]` only adds
            # LLM results.
            # If a page was a full cache hit, it's not in pending_page_cache_updates.
            # If a page was a full cache miss, pending_page_cache_updates[page_hash]
            # contains all its items.
            # If a page was a partial hit (some runs from cache, some to LLM):
            #   - Runs from cache are already applied.
            #   - Runs to LLM have their translations in
            #     pending_page_cache_updates[page_hash].
            #   - We need to ensure the final cache entry for this page_hash contains
            #     *all* original_text/translation pairs.

            # Simplest approach for now: if a page_hash is in
            # pending_page_cache_updates, it means it had at least one LLM translation.
            # We'll build its cache entry from all_processed_run_details that belong to
            # that page_hash.

            # Rebuild the cache entry for any page that had LLM involvement or was a
            # full miss.
            if (
                page_hash in pending_page_cache_updates
            ):  # Indicates LLM was involved for this page
                # Find all runs associated with this page_hash from
                # all_processed_run_details
                # This is inefficient if done here. Better to build
                # pending_page_cache_updates correctly during LLM response processing.
                # For now, let's assume pending_page_cache_updates[page_hash] has the
                # full list for pages that had misses/LLM calls.
                # The current logic for populating pending_page_cache_updates might be
                # okay if it collects all items for a page_hash that had any LLM calls.

                # Let's refine: pending_page_cache_updates should store the *complete*
                # list of translations for a page if it's being updated.
                ...
                # If a page was a cache miss, all its items went to LLM, so
                # pending_page_cache_updates[page_hash] will be complete.
                # The tricky case is a "partial page cache hit" where the hash matched,
                # but some texts were new. In this scenario,
                # `translation_cache[page_hash]` (the old entry) needs to be
                # augmented/replaced.
                # The current logic for "partial page cache hit" adds new items to
                # global_texts_for_llm_prompt, and their translations will be added to
                # pending_page_cache_updates[page_hash]. We need to merge these with the
                # *original* cached items for that page that were hits.

                # Revised strategy for updating translation_cache:
                # For any page_hash that appears in pending_page_cache_updates:
                #   Construct its new cache entry by taking all items from all_processed
                #   run_details that correspond to that page_hash (need to add page_hash
                # to all_processed_run_details items).
                # This is complex. Let's simplify: if a page_hash is in
                # pending_page_cache_updates, it means it was either a full miss, or a
                # partial miss where new items were sent to LLM.
                # The `pending_page_cache_updates[page_hash]` should contain the *newly
                # translated* items.
                # If it was a full miss, this is the complete list for the page.
                # If it was a partial miss, we need to merge with existing cached items
                # for that page.

                # Let's stick to the plan: `pending_page_cache_updates[page_hash]`
                # stores the *full list* for pages that had misses.
                # The LLM response loop correctly adds
                # `{"original_text": ..., "translation": ...}` to
                # `pending_page_cache_updates[current_page_hash]`.
                ...
                if translations_list:  # Only update if there are actual translations
                    ...
                    # This means a page hash was in pending_updates (so it was a miss or
                    # partial) but ended up with no translations (e.g. LLM returned
                    # nothing for its items). We should probably remove it from cache to
                    # force re-translation next time, or ensure
                    # pending_page_cache_updates is built correctly with all original
                    # texts even if translations are empty.
                    # For now, if translations_list is empty, we don't add/update it,
                    # which means if it was a full miss, it won't be cached. If it was
                    # partial, the old cache entry might persist if not explicitly
                    # overwritten. This needs to be robust: if a page is processed and
                    # had LLM calls, its entry in translation_cache should reflect the
                    # latest state of all its texts.

                    # Corrected logic: if page_hash is in pending_page_cache_updates, it
                    # means we intended to update it.
                    # The list `translations_list` should be the definitive new list of
                    # original_text/translation pairs for that page.
                    translation_cache[page_hash] = translations_list
